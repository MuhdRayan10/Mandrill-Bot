...
class Genesis(commands.Cog):
    ...
    @app_commands.command(
        name="setup-genesis", description="[MODS] Setup Genesis embed"
    )
    async def setup_genesis(self, interaction, channel: discord.TextChannel):
        await interaction.response.send_message(
            f"Sending in {channel.mention}", ephemeral=True
        )
    ...
    async def genesis_callback(self, interaction):
        ...
        embed = discord.Embed(title="Click the button to verify", color=Var.base_color)

        view = ui.View()
        btn1 = ui.Button(
            label="Connect",
            style=discord.ButtonStyle.green,
            url=f"https://www.themandrills.xyz/verify.php?discord={interaction.user.id}",
        )
        btn = ui.Button(label="Get Genesis!", style=discord.ButtonStyle.blurple)
    ...
    async def verify_callback(self, interaction):
        ...
        if not bool_:
            embed = discord.Embed(
                title="Genesis Role is not Assigned!",
                color=Var.base_color,
