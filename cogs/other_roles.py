...
class Roles(commands.Cog):
    def __init__(self, bot):
        ...
        criteria = ui.Button(
            label="Criteria",
            style=discord.ButtonStyle.blurple,
            custom_id="requirements_prom:blurple",
        )
    ...
    async def warning(self, interaction):
        ...
        embed.add_field(
            ...
            value="""- You have to answer all questions correctly in order to get the Promdrill role
...
- Read carefully, don’t rush and Good Luck!""",
    ...
    async def give_explorill(self, interaction):
        ...
        if unverified_role in roles:
            await interaction.response.send_message(
                f"Unfortunately you are still unverified... Go verify yourself at <#{Var.verification_channel}>",
                ephemeral=True,
            )
            return
        ...
        if explorill_role in roles:
            ...
            await interaction.response.send_message(
                f"You are now officially an Explorill!", ephemeral=True
            )
            await interaction.user.remove_roles(
                interaction.guild.get_role(Var.muted_role)
            )

    @app_commands.checks.has_any_role(Var.guardrill_role, Var.liberator_role)
    @app_commands.command(
        name="setup-explorill", description="[MODS] Setup explorill role interface"
    )
    ...
    @app_commands.checks.has_any_role(Var.guardrill_role, Var.liberator_role)
    @app_commands.command(
        name="setup-promdrill", description="[MODS] Sets up promdrill interface."
    )
    ...
    async def promdrill(self, interaction):
        ...
        if passed:
            result_embed = discord.Embed(
                title="Rendrill Questionnaire Results", color=Var.base_color
            )

            result_embed.description = (
                "Congratulations, you have passed the questionnaire!"
            )
        else:
            result_embed = discord.Embed(
        ...
        if passed:
            ...
            result_embed.description = (
                "Sorry, you have failed the questionnaire. Better luck next time."
            )
        ...
        if passed:
            ...
            await interaction.followup.send(
                "You have been awarded the `Promdrill` Role!", ephemeral=True
            )
            return
    ...
    async def view(self, interaction):
        ...
        # Embed
        embed = discord.Embed(
            title="Promdrill Role Criteria",
            description="Complete all 3 tasks to get the Promdrill role!",
            color=Var.base_color,
        )
        embed.add_field(
            name=f"{rc if data[1] < 8 else wc} Invite at least 8 users to the server",
            value="ㅤ",
            inline=False,
        )
        embed.add_field(
            name=f"{rc if data[2] < 12 else wc} Reach Level - 12",
            value="ㅤ",
            inline=False,
        )
        embed.add_field(
            name=f"{rc} Complete the Quiz (after 1st & 2nd tasks)",
            value="ㅤ",
            inline=False,
        )
    ...
    @app_commands.command(
        name="set-req-prom",
        description="[MODS] Set requirements for a user to obtain Promdrill role.",
    )
    ...
    async def set_req_prom(
        self, interaction, user: discord.Member, activity: int, done: int
    ):
        # add to json file
