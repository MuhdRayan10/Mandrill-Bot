...
class TicketSettings(View):
    ...
    async def close(self, interaction, _):
        await interaction.response.send_message(
            "Ticket is being closed...", ephemeral=True
        )
...
class Tickets(commands.Cog):
    def __init__(self, bot):
        ...
        button = discord.ui.Button(
            label="Proceed", style=discord.ButtonStyle.green, custom_id="ticket:green"
        )
    ...
    @app_commands.command(
        name="setup-tickets", description="[MODS] Setup Ticket Interface"
    )
    @app_commands.describe(
        channel="The channel where the Ticket Interface is to be set up"
    )
    async def setup_tickets(self, interaction, channel: discord.TextChannel):
        embed = discord.Embed(
            title="Create a Ticket!",
            description="Click on the `Proceed` button below to create a ticket. One of our team member will be notified and shortly aid you with your problem.",
    ...
    async def create_ticket(self, interaction):
        ...
        msg = await interaction.followup.send(
            content="A ticket is being created!", ephemeral=True
        )
        ...
        category = discord.utils.get(
            interaction.guild.categories, id=1052257859112538213
        )
        ...
        await interaction.followup.edit_message(
            msg.id, content=f"Channel created successfully! {channel.mention}"
        )
