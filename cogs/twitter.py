...
class TwitterCog(commands.Cog):
    def __init__(self, bot):
        ...
        db.create_table(
            "users",
            {"user": "INTEGER", "name": "TEXT", "twitter": "TEXT", "wallet": "TEXT"},
        )
    ...
    @app_commands.command(
        name="setup-purmarill",
        description="Setup the Purmarill Interface in the specified channel",
    )
    async def setup_purmarill(self, interaction, channel: discord.TextChannel):
        ...
        await interaction.response.send_message(
            f"Added `get prumarill` app, to <#{channel.id}>"
        )
    ...
    async def link(self, interaction):
        ...
        if not whitelisting:
            await interaction.response.send_message(
                embed=discord.Embed(
                    title="Unfortunately, Purmarill Whitelisting is already closed"
                ),
                ephemeral=True,
            )
            return
        ...
        class PurmarillVerificationModal(ui.Modal, title="Purmarill Verification"):
            ...
            async def on_submit(self, interaction: discord.Interaction) -> None:
                ...
                if role in interaction.user.roles:
                    await interaction.response.send_message(
                        "Twitter account / Wallet ID already registered...",
                        ephemeral=True,
                    )
                    return
                ...
                if not validify_twitter(str(self.twitter_username)):
                    await interaction.response.send_message(
                        "Twitter username does not exist.", ephemeral=True
                    )
                    return
                ...
                if not validify_wallet(str(self.wallet_id)):
                    await interaction.response.send_message(
                        "FLR address is not valid.", ephemeral=True
                    )
                    return
                ...
                db.insert(
                    ...
                    (
                        ...
                        str(self.twitter_username),
                        str(self.wallet_id),
                    ),
                )
                await interaction.response.send_message(
                    "You are now officially a Purmarill!", ephemeral=True
                )
    ...
    async def partnership_embed(self, interaction):
        ...
        embed = discord.Embed(
            title="Before you submit...", description=desc, color=Var.base_color
        )
        view = ui.View()
    ...
    async def update_wallet(self, interaction, address: str):
        ...
        if not data:
            await interaction.response.send_message(
                "Looks like there is no Wallet address currently linked to your account...",
                ephemeral=True,
            )
            return
        ...
        if not Web3.isAddress(address):
            await interaction.response.send_message(
                "Invalid Wallet Address", ephemeral=True
            )
            return

        db.update(
            "users",
            information={"wallet": address},
            where={"user": interaction.user.id},
        )

        await interaction.response.send_message(
            f"Succesfully updated wallet address from `{data[3]}` to `{address}`!",
            ephemeral=True,
        )
