...
class XP(commands.Cog):
    def __init__(self, bot):
        ...
        db.create_table(
            "levels", {"user": INT, "level": INT, "xp": INT, "lim": INT, "total": INT}
        )
    ...
    def spam(self, user, time_):
        if user in self.spam_cache:
            return (
                True
                if self.spam_cache[user][0] == time_ and self.spam_cache[user][1] >= 3
                else False
            )
    ...
    async def on_message(self, message):
        ...
        if self.spam(auth_id, time_):
            return

        new_time = (
            self.spam_cache[auth_id][1] + 1
            if time_ == self.spam_cache[auth_id][0] == time_
            else 1
        )
        ...
        if levels[2] + xp >= levels[3]:
            db.update(
                ...
                {
                    "level": levels[1] + 1,
                    "xp": levels[2] + xp - levels[3],
                    "lim": int(Var.level_difficult_factor * levels[3]),
                    "total": levels[4] + xp,
                },
                where={"user": auth_id},
            )
            ...
            if current_data[1] >= 4 and current_data[2] >= 8 and not current_data[3]:
                await message.reply(
                    f"Looks like you are almost eligible for the `Rendrill` role! To complete the quiz, go to <#{Var.rendrill_channel}> and click on the `GET RENDRILL` button to start the quiz!"
                )

        else:
            db.update(
                "levels",
                {"xp": levels[2] + xp, "total": levels[4] + xp},
                where={"user": auth_id},
            )
    ...
    async def level(self, interaction, user: discord.Member = None):
        if interaction.channel.id != Var.command_channel:
            await interaction.response.send_message(
                f"Please use <#{Var.command_channel}> here", ephemeral=True
            )
            return
        ...
        bg.text(
            (200, 130),
            f"Level - {data[1]} | XP - {data[2]}/{data[3]}",
            font=poppins_small,
            color="#FFFFFF",
        )
    ...
    @app_commands.command(
        name="leaderboard", description="View the server's Top 10 Users"
    )
    async def leaderboard(self, interaction):
        ...
        plt.pie(
            [d[1] for d in data],
            labels=[f"#{i} {d[0]}" for i, d in enumerate(data, start=1)],
            autopct="%1.2f%%",
        ...
        with io.BytesIO() as image_binary:
            ...
            embed = discord.Embed(
                title="The Mandrills' XP Leaderboard", color=Var.base_color
            )
            embed.set_image(url="attachment://image.png")

            await interaction.response.send_message(
                embed=embed, file=discord.File(fp=image_binary, filename="image.png")
            )

        plt.close()

    @app_commands.command(
        name="invites", description="See how many invites the user has."
    )
    ...
    async def invites(self, interaction, user: discord.Member = None):
        ...
        if not data:
            await interaction.response.send_message(
                embed=discord.Embed(
                    title=f"{user.name} has no invites.", color=Var.base_color
                )
            )
            return
