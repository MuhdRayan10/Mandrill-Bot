...
class Moderation(commands.Cog):
    ...
    async def kick(self, interaction, user: discord.Member, reason: str):
        ...
        await interaction.response.send_message(
            f"{user.name} has been kicked by {interaction.user.mention} for the reason: `{reason}`!"
        )
    ...
    async def ban(self, interaction, user: discord.Member, reason: str):
        ...
        await interaction.response.send_message(
            f"{user.name} has been banned by {interaction.user.mention} for the reason: `{reason}`!"
        )

    @app_commands.checks.has_permissions(ban_members=True)
    @app_commands.command(
        name="softban", description="[MODS] Softbans (resets) a user from the server"
    )
    ...
    async def softban(self, interaction, user: discord.Member, reason: str):
        ...
        await interaction.response.send_message(
            f"{user.name} has been softbanned by {interaction.user.mention} for the reason: `{reason}`!"
        )

    @app_commands.checks.has_permissions(ban_members=True)
    @app_commands.command(
        name="tempban", description="[MODS] Temp ban  a user from the server"
    )
    ...
    async def tempban(
        self, interaction, user: discord.Member, reason: str, time: int = 5
    ):
        await user.ban(reason=reason)
        await interaction.response.send_message(
            f"{user.name} has been temp banned by {interaction.user.mention} for the reason: `{reason}` for {time} minutes!"
        )
    ...
    async def tempmute(self, interaction, user: discord.Member, time: int):
        muted_role = interaction.guild.get_role(Var.muted_role)
        await interaction.response.send_message(
            f"Temp Muted {user.mention} for {time} minutes!"
        )
        await asyncio.sleep(time * 60)
        await user.remove_roles(muted_role)
    ...
    async def on_message(self, message):
        ...
        if message.channel.id == Var.command_channel:
            await message.reply(
                content=f"**⛔ Sorry. Messages are not allowed in <#{Var.command_channel}>!**",
                delete_after=10,
            )
            await message.delete()
        ...
        for keyword in keywords:
            if keyword in message.content:
                await message.reply(
                    content="**⛔ Sorry. Links and Embeds are not allowed in our Server!**",
                    delete_after=10,
                )
                await message.delete()
